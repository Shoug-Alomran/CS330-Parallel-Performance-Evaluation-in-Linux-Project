# Project Team

This project is completed as a group effort for the **CS330: Introduction to Operating Systems** course. Each team member contributes to different aspects of the project, including setup, experimentation, documentation, and analysis.

## Team Members

- **Shoug Alomran**  
  Student ID: 223410392

- **Layan Bin Dayel**  
  Student ID: 223410104

- **Aryam Yahya Almansour**  
  Student ID: 223410070

- **Danah Altuwaijri**
  Student ID:223410151

- **Rana Alfaris**
  Student ID:223410567


## Collaboration

The team collaborates on planning, execution, and documentation to ensure consistency across all project phases. Responsibilities are distributed to maintain a balanced workload while meeting the technical and academic requirements of the course.

---
**Project Plan Dashboard (Roles & Tasks):** [Open the Roles Dashboard](./Project Plan.html)


This page serves as the official record of team membership for the project.
